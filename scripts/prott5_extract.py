...
MODEL_DIR = ROOT / "hf_models" / "prot_t5_xl_half_uniref50-enc"
...
# ProtT5 特征提取
# =========================
def extract_prott5_features(ids, sequences, batch_size=BATCH_SIZE):
    ...
    for batch_ids, batch_seqs in batch_iter(ids, sequences, batch_size):
        ...
        residue_lens = [len(seq) for seq in batch_seqs]
        ...
        for i, seq_len in enumerate(residue_lens):
            # 只取真实残基位置，排除 padding 影响
            residue_embeddings = hidden[i, :seq_len, :]
            emb = residue_embeddings.mean(dim=0).detach().float().cpu().numpy()
...
def main():
    ...
    kept_ids, features = extract_prott5_features(
        ids=ids,
        sequences=sequences,
    ...
    feature_cols = [f"ProtT5_{i}" for i in range(feature_dim)]

    feature_csv = OUT_DIR / f"ProtT5_all_{feature_dim}.csv"
    feature_npy = OUT_DIR / f"ProtT5_all_{feature_dim}.npy"
    meta_csv = OUT_DIR / "ProtT5_ids.csv"
