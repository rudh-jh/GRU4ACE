...
SAVE_DIR = BASE_DIR / "models" / "GRU_EN_306_SMOTE"
...
def main():
    ...
    print(f"[INFO] SMOTE train shape: {X_train_bal.shape}")
    print(f"[INFO] SMOTE train labels: pos={(y_train_bal == 1).sum()}, neg={(y_train_bal == 0).sum()}")

    # 保存中间数据
    train_bal_df = pd.DataFrame(X_train_bal)
    train_bal_df["label"] = y_train_bal
    train_bal_df.to_csv(SAVE_DIR / "XtrainData_SMOTE.csv", index=False)
    ...
    print("\n[INFO] Start 5-fold cross validation with SMOTE")
